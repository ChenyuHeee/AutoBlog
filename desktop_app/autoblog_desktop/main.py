...
class DeployWorker(QtCore.QThread):
    ...
    def run(self) -> None:
        try:
            ...
            original_run_git = autoblog_build.run_git
            ...
            def run_git_captured(
                ...
                # Always capture subprocess output so it can be forwarded into the GUI log.
                result = original_run_git(command, cwd=cwd, capture_output=True)
            ...
            autoblog_build.run_git = run_git_captured  # type: ignore[assignment]

            buf = io.StringIO()
            try:
                with contextlib.redirect_stdout(buf):
                    config = autoblog_build.build_site(args)
                    autoblog_build.deploy_to_github(config, args)
            finally:
                autoblog_build.run_git = original_run_git  # type: ignore[assignment]
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, project: Project):
        ...
        self.deploy_worker: DeployWorker | None = None
    ...
    def _init_build_tab(self) -> None:
        ...
        self.btn_deploy = QtWidgets.QPushButton("部署")
        ...
        deploy_opts = QtWidgets.QHBoxLayout()
        self.chk_deploy_dry_run = QtWidgets.QCheckBox("部署演练（不 push）")
        self.chk_deploy_dry_run.setChecked(True)
        self.edt_deploy_branch = QtWidgets.QLineEdit()
        self.edt_deploy_branch.setPlaceholderText("branch 覆盖（留空使用 config 的 github_branch）")
        self.edt_deploy_message = QtWidgets.QLineEdit()
        self.edt_deploy_message.setPlaceholderText("commit message（留空自动生成）")
        deploy_opts.addWidget(self.chk_deploy_dry_run)
        deploy_opts.addSpacing(10)
        deploy_opts.addWidget(QtWidgets.QLabel("branch"))
        deploy_opts.addWidget(self.edt_deploy_branch, 1)
        deploy_opts.addSpacing(10)
        deploy_opts.addWidget(QtWidgets.QLabel("message"))
        deploy_opts.addWidget(self.edt_deploy_message, 2)
        layout.addLayout(deploy_opts)
        ...
        self.btn_deploy.clicked.connect(self._run_deploy)
    ...
    def _run_deploy(self) -> None:
        ...
        include_drafts = self.chk_drafts.isChecked()
        skip_clean = self.chk_skip_clean.isChecked()
        dry_run = self.chk_deploy_dry_run.isChecked()
        branch = self.edt_deploy_branch.text().strip() or None
        message = self.edt_deploy_message.text().strip() or None

        if not dry_run:
            confirm = QtWidgets.QMessageBox(self)
            confirm.setIcon(QtWidgets.QMessageBox.Icon.Warning)
            confirm.setWindowTitle("确认部署")
            confirm.setText("即将执行部署（会 git push 到远端）。继续吗？")
            detail = [
                f"include_drafts={include_drafts}",
                f"skip_clean={skip_clean}",
                f"branch_override={branch or '(use config)'}",
                f"message={message or '(auto)'}",
            ]
        ...
        self._append_log("\n=== Deploy: {} ===".format(dt.datetime.now().isoformat(timespec="seconds")))
        self._append_log(
            f"include_drafts={include_drafts} skip_clean={skip_clean} dry_run={dry_run} "
            f"branch={branch or ''} message={(message or '')}"
        )
        ...
        self.deploy_worker.finished_ok.connect(self._on_deploy_ok)
        self.deploy_worker.finished_err.connect(self._on_deploy_err)
    ...
    def _on_deploy_ok(self, output: str) -> None:
        self._append_log(output or "(no output)")
        self._toast("部署完成")

    def _on_deploy_err(self, err: str) -> None:
        self._append_log(f"[ERROR] {err}")
        msg = QtWidgets.QMessageBox(self)
        msg.setIcon(QtWidgets.QMessageBox.Icon.Critical)
        msg.setWindowTitle("部署失败")
        msg.setText(err)
        msg.exec()
