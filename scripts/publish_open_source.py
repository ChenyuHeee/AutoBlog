...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Write the sanitized snapshot to this directory (useful for review). "
...
def strip_dot_slash_prefix(value: str) -> str:
    value = value.strip()
    if value.startswith("./"):
        return value[2:]
    return value
...
def should_skip(rel_path: Path, patterns: list[str]) -> bool:
    ...
    if rel_str == ".":
        return False
    rel_str = strip_dot_slash_prefix(rel_str)
    if not rel_str:
        return False
    ...
    for pattern in patterns:
        norm = strip_dot_slash_prefix(pattern)
        if not norm:
            continue

        # Treat "dir/**" as excluding the directory itself too.
        if norm.endswith("/**"):
            base = norm[: -len("/**")]
            if rel_str == base or rel_str.startswith(f"{base}/"):
                return True
...
def main() -> None:
    ...
    if args.out:
        snapshot_dir = (ROOT / args.out).resolve() if not os.path.isabs(args.out) else Path(args.out)
        snapshot_dir.parent.mkdir(parents=True, exist_ok=True)
        tmp_ctx = None
    else:
        tmp_ctx = tempfile.TemporaryDirectory(prefix="autoblog-publish-")
    ...
    try:
        ...
        if args.dry_run:
            ...
            print(f"[publish] Snapshot dir: {snapshot_dir}")
